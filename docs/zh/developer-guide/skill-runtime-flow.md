...
- `requestedSkillRefs` 是新的原始选择，保存 `name/namespace/is_public`，并可选保存 `skill_id`

其中 `skill_id` 不是必填字段，但当用户在聊天界面选择的技能存在“跨组同名”或“同 namespace 不同 owner”歧义时，backend 会把当次显式选择对应的 `skill_id` 一并持久化，作为更精确的选择真相源。

如果历史任务没有这个字段，或者该字段失效，backend 仍然会按 `name/namespace/is_public` 重新解析，不要求旧数据回填。
...
聊天界面的技能选择器和统一技能列表接口还会保留“同名但不同 namespace / owner”的多个条目。也就是说，前端不能再把 `name` 当成唯一键，而必须把完整 skill ref 透传给 backend。
...
3. 新 Task 会同时写入 `requestedSkillRefs`，并在可用时附带 `skill_id`，保证普通聊天在 sandbox / 本地设备启动期也能找回用户当时选择的命名空间和精确技能实例
