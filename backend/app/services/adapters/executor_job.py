...
class JobService(BaseService[Kind, None, None]):
    ...
    def cleanup_stale_executors(self, db: Session) -> None:
        ...
        try:
            ...
            cursor = executor_cleanup_cursor_service.get_cursor(db)
            ...
            while (
                ...
                scanned_subtasks = self._scan_candidate_subtasks_batch(
                    db=db,
                    last_id=last_id,
                ...
                executor_cleanup_cursor_service.advance_cursor(
                    db,
    ...
    def _scan_candidate_subtasks_batch(
        self,
        db: Session,
        *,
        last_id: int,
    ...
    def _scan_lookback_subtasks_batch(
        self,
        db: Session,
        *,
    ...
    def _process_cleanup_batch(
        self, db: Session, *, candidates: List[Subtask], chat_cutoff: datetime
        ...
        task_map = self._load_tasks_for_cleanup(
            db=db,
        ...
        deleted_count = self._cleanup_executor_groups(
            db=db,
    ...
    def _cleanup_executor_groups(
        ...
        *,
        ...
        for task_id in task_ids:
            ...
            try:
                result = self._cleanup_executor_entries(
                    db=db,
                    task_id=task_id,
                    task=task,
                    subtasks=subtasks,
    ...
    def _archive_workspace_sync(
        ...
        try:
            ...
            if archive_info:
                ...
                logger.info(
                    f"[executor_job] Workspace archiving skipped or failed "
                    f"task_id={task.id}"
                )
        finally:
